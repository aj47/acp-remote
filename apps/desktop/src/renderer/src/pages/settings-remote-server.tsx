        ...
        <ControlGroup
          ...
          endDescription={(
            <div className="break-words whitespace-normal">
              Exposes your ACP Remote agent over an OpenAI BaseURL-compatible /v1 HTTP endpoint so other clients (e.g., mobile or other apps) can connect to this desktop app and use the agent remotely. Recommended: use with the{" "}
              <a
                href="https://github.com/aj47/acp-remote"
                target="_blank"
                rel="noreferrer noopener"
                className="underline"
              >
                ACP Remote Mobile app
              </a>.
            </div>
          )}
        >
              ...
              {baseUrl && (
                <>
                  ...
                  {cfg?.remoteServerApiKey && (
                    <Control label={<ControlLabel label="Mobile App QR Code" tooltip="Scan this QR code with the ACP Remote mobile app to connect (local network only)" />} className="px-3">
                      <div className="flex flex-col items-start gap-3">
                        {streamerMode ? (
                          ...
                          <div className="p-3 bg-white rounded-lg">
                            <QRCodeSVG
                              value={`acpremote://config?baseUrl=${encodeURIComponent(baseUrl)}&apiKey=${encodeURIComponent(cfg.remoteServerApiKey)}`}
                              size={160}
                              level="M"
                            />
                          </div>
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          <Button
                            ...
                            onClick={() => {
                              ...
                              const deepLink = `acpremote://config?baseUrl=${encodeURIComponent(baseUrl)}&apiKey=${encodeURIComponent(cfg.remoteServerApiKey || "")}`
                        ...
                        <div className="text-xs text-muted-foreground">
                          {streamerMode
                            ...
                            : "Scan with the ACP Remote mobile app to auto-configure. Works on local network only. For internet access, use Cloudflare Tunnel below."}
                        </div>
                      </div>
                    </Control>
                  )}
              ...
              <>
                ...
                {tunnelStatus?.url && tunnelStatus?.running && (
                  <>
                    ...
                    {cfg?.remoteServerApiKey && (
                      <Control label={<ControlLabel label="Mobile App QR Code" tooltip="Scan this QR code with the ACP Remote mobile app to connect" />} className="px-3">
                        <div className="flex flex-col items-start gap-3">
                          {streamerMode ? (
                            <div className="p-3 bg-muted/50 rounded-lg flex flex-col items-center justify-center" style={{ width: 160, height: 160 }}>
                              <EyeOff className="h-8 w-8 text-muted-foreground mb-2" />
                            ...
                            <div className="p-3 bg-white rounded-lg">
                              <QRCodeSVG
                                value={`acpremote://config?baseUrl=${encodeURIComponent(`${tunnelStatus.url}/v1`)}&apiKey=${encodeURIComponent(cfg.remoteServerApiKey)}`}
                                size={160}
                                level="M"
                              />
                            </div>
                          ...
                          <div className="flex flex-wrap items-center gap-2">
                            <Button
                              ...
                              onClick={() => {
                                if (streamerMode) return
                                const deepLink = `acpremote://config?baseUrl=${encodeURIComponent(`${tunnelStatus.url}/v1`)}&apiKey=${encodeURIComponent(cfg.remoteServerApiKey || "")}`
                                navigator.clipboard.writeText(deepLink)
                              }}
                            >
                              {streamerMode ? <><EyeOff className="h-3.5 w-3.5 mr-1" />Hidden</> : "Copy Deep Link"}
                          ...
                          <div className="text-xs text-muted-foreground">
                            {streamerMode
                              ? "QR code and deep link hidden in Streamer Mode"
                              : "Scan with the ACP Remote mobile app to auto-configure the connection."}
                          </div>
                        </div>
                      </Control>
                    )}
