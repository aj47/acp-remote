 ...
 *
 * @param toolName - The full tool name (e.g., 'acpremote-builtin:list_available_agents')
...
export async function executeACPRouterTool(
  ...
  try {
    ...
    switch (resolvedToolName) {
      case 'acpremote-builtin:list_available_agents':
        result = await handleListAvailableAgents(args as { capability?: string; skillName?: string });
        break;

      case 'acpremote-builtin:delegate_to_agent':
      ...
      case 'acpremote-builtin:check_agent_status':
