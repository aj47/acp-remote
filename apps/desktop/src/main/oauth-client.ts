...
export class OAuthClient {
  ...
  async registerClient(): Promise<{ clientId: string; clientSecret?: string }> {
    ...
    const clientMetadata: OAuthClientMetadata = {
      ...
      ...(this.config.clientMetadata && {
        client_name: this.config.clientMetadata.client_name || 'ACP Remote',
  ...
  private getRedirectUri(): string {
    ...
    return isDevelopment ? 'http://localhost:3000/callback' : 'acpremote://oauth/callback'
  }
